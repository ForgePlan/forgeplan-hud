...
### Live refresh при `forgeplan`-командах

`install.sh` также прописывает **PostToolUse:Bash хук** (`daemon/invalidate.sh`)
в `~/.claude/settings.json`. После любого Bash-вызова с `forgeplan` или `fpl`
хук удаляет cache stamp — на следующем тике statusline (≤10 с, обычно
следующее сообщение ассистента) демон форкается и обновляет кэш за ~200-500 мс.

Это превращает HUD в **tight feedback loop**: запустил `forgeplan link PRD-001 ...` →
на следующем ответе бар уже отражает новый счётчик orphans. Без хука пришлось бы
ждать 30 с TTL кэша.

Матчер срабатывает только на standalone слова `forgeplan` / `fpl` — `forgeplan-hud`
и `myfpl` корректно игнорируются. `uninstall.sh` убирает хук чисто.
